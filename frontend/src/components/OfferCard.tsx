...
interface Props { offer: OfferResponseDto; }

/* map (sub-)strings → logo image */
...
export default function OfferCard({ offer }: Props) {
  const { productId, provider, contractInfo, costInfo, tvInfo } = offer;
  ...
  /* price/discount */
  const hasDiscount =
    costInfo.monthlyDiscountValueInCent != null &&
    costInfo.monthlyDiscountValueInCent > 0 &&
  ...
  /* rating */
  const rating = offer.avgRating ?? 0;
    ...
    <div className="card bg-base-100 shadow-md hover:shadow-xl transition duration-300">
      <div className="card-body p-4 flex flex-row items-center gap-6">
        ...
        <div className="flex flex-col items-center w-28 shrink-0">
          <span className="text-xl font-bold text-primary font-bold mb-1">
            {rating.toFixed(1)}/5
          </span>
          ...
          <div className="flex flex-wrap gap-1 text-xs opacity-80">
            ...
            {contractInfo.speedLimitFrom && (
              <span className="badge badge-outline">
                Limitation From {contractInfo.speedLimitFrom} GB
              </span>
            )}
        ...
        <div className="flex flex-col items-end w-48 shrink-0">
          {hasDiscount && (
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-error font-semibold">
                -€{euro(costInfo.monthlyDiscountValueInCent!)}
              </span>
            </div>
          )}
          ...
          {costInfo.monthlyCostAfter24mInCent && (
            <span className="text-xs opacity-70 mt-1">
              from month 25: €{euro(costInfo.monthlyCostAfter24mInCent)} / Month
            </span>
          )}
